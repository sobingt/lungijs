...
exports.postVerifyEmail = function(req, res, next) {
  req.body.email = req.body.email.toLowerCase();
  User.findOne(req.body, function(err, user) {
    if (!user) {
      return res.status(401).send({ message: 'Invalid email and/or password', status:  'auth-failed' });
    }
    ...
    if(user.status == "verification-email")
    {
      ...
      return res.status(401).send({ 
        message: 'Oops we have got server problem. Try after sometime',
        status:  'error' 
      });
  });
}
...
|--------------------------------------------------------------------------
*/
exports.postInstagramLogin = function (req, res) {
...
/*
 ...
 |--------------------------------------------------------------------------
 */
exports.postLiveLogin = function (req, res) {
  async.waterfall([
    // Step 1. Exchange authorization code for access token.
    function (done) {
...
/*
 ...
 |--------------------------------------------------------------------------
 */
exports.postTwitchLogin = function (req, res) {
...
/*
 ...
 |--------------------------------------------------------------------------
 */
exports.postBitBucketLogin = function (req, res) {
      ...
      request.get({
        ...
        if (req.header('Authorization')) {
          ...
          User.findOne({
            ...
            //next to creating new user with req.userData
            return next();
